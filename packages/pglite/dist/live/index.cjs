...
`)},
            ...
            `)}P=await Promise.all(d.map(E=>p.listen(`"table_change__${E.schema_oid}__${E.table_oid}"`,async()=>{D()})))})};await R();let D=ne(async()=>{if(u.length===0&&m)return;let p=!1;for(let o=0;o<5;o++)try{await e.transaction(async d=>{await d.exec(`
    ...
    `,[t])).rows.map(s=>({table_name:s.table_name,schema_name:s.schema_name,table_oid:s.table_oid,schema_oid:s.schema_oid}))}async function pe(e,t,n){let s=t.filter(r=>!n.has(`${r.schema_oid}_${r.table_oid}`)).map(r=>`
      ...
      `).join(`
`);s.trim()!==""&&await e.exec(s),t.map(r=>n.add(`${r.schema_oid}_${r.table_oid}`))}var z=(e,t)=>{for(let n of e)n(t)},ut=(e,t)=>{for(let n of e)n(t)};0&&(module.exports={live});
